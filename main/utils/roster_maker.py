...
def split_leaders_and_members(all_members):
    ...
    for m in all_members:
        if m.get_datafield("Role") == "Member":
            members.append(m)
        else:
            leaders.append(m)
...
def sort_block_out_dates_priority(members, block_out_dates):
    ...
    for m in members:
        ...
        for d in list(block_out_dates.get_datafield("Block Out Dates").keys()):
            if m.get_datafield("Name") in block_out_dates.get_datafield("Block Out Dates")[d]:
...
def has_a_leader(serving_week):
    for m in serving_week:
        if not m.get_datafield("Role") == "Member":
            return True
    return False
...
def has_a_leader_in_all_weeks(roster):
    res = True
    for d in list(roster.get_datafield("Roster").keys()):
        res &= has_a_leader(roster.get_datafield("Roster")[d])
    return res
...
def is_roster_full(roster, min_serve_members):
    res = True
    for d in list(roster.get_datafield("Roster").keys()):
        res &= is_serving_week_full(roster.get_datafield("Roster")[d], min_serve_members)
    return res


def get_person_serving_amount(roster, person):
    return sum([1 for d in roster.get_datafield("Roster").keys() if
                person.get_datafield("Name") in roster.get_datafield("Roster")[d]])
...
def process_new_roster(roster):
    schedule = {}
    for d in sorted(roster.get_datafield("Roster").keys(), key=lambda x: int(x)):
        schedule[d] = [member.get_datafield("Name") for member in roster.get_datafield("Roster")[d]]
    roster.set_datafield(field="Roster", data=schedule)
    return roster
...
def roster_maker(block_out_dates, all_members, prev_roster, min_serve_members=3):
    ...
    if prev_roster != {}:
        last_month_date = sorted(
            prev_roster.get_datafield("Roster").keys(),
            key=lambda x: int(x)
        )[-1]
        last_month_serving_week = prev_roster.get_datafield("Roster")[last_month_date]
    serving_dates = sorted(
        block_out_dates.get_datafield("Block Out Dates").keys(),
        key=lambda x: int(x)
    )
    serving_members = []
    for m in all_members:
        can_serve = False
        for d in list(block_out_dates.get_datafield("Block Out Dates").keys()):
            if m.get_datafield("Name") not in block_out_dates.get_datafield("Block Out Dates")[d]:
                can_serve |= True
        if can_serve:
            serving_members.append(m)
    roster = Roster(
        month=str(block_out_dates.get_datafield("Month")),
        year=int(block_out_dates.get_datafield("Year"))
    )
    schedule = {}
        ...
        if is_roster_full(roster, min_serve_members):
            ...
            for d in range(len(serving_dates)):
                ...
                is_member_available = still_has_members and (
                    one_time_members[0].get_datafield("Name") not in
                    block_out_dates.get_datafield("Block Out Dates")[date]
                )
                is_leader_available = still_has_leaders \
                    and (
                        one_time_leaders[0].get_datafield("Name") not in
                        block_out_dates.get_datafield("Block Out Dates")[date]
                    )
                is_member_in_prev_week = still_has_members \
                    and (
                        (is_first_date and (one_time_members[0].get_datafield("Name") in last_month_serving_week))
                        or (
                            (not is_first_date)
                            and (
                                one_time_members[0].get_datafield("Name") in
                                roster.get_datafield("Roster")[serving_dates[d - 1]]
                            )
                        )
                    )
                is_member_in_next_week = still_has_members \
                    and (
                        (is_last_date and (one_time_members[0].get_datafield("Name") in []))
                        or (
                            (not is_last_date)
                            and (one_time_members[0].get_datafield("Name")in roster.get_datafield("Roster")[serving_dates[d + 1]])
                        )
                    )
                is_leader_in_prev_week = still_has_leaders \
                    and (
                        (
                            is_first_date and (one_time_leaders[0].get_datafield("Name") in last_month_serving_week)
                        ) or (
                            (not is_first_date)
                            and (one_time_leaders[0].get_datafield("Name") in roster.get_datafield("Roster")[serving_dates[d - 1]])
                        )
                    )
                is_leader_in_next_week = still_has_leaders \
                    and (
                        (is_last_date and (one_time_leaders[0].get_datafield("Name") in []))
                        or (
                            (not is_last_date)
                            and (one_time_leaders[0].get_datafield("Name")in roster.get_datafield("Roster")[serving_dates[d + 1]])
                        )
                    )
                if not is_serving_week_full(roster.get_datafield("Roster")[date], min_serve_members):
                    if not has_a_leader(roster.get_datafield("Roster")[date]):
                        if still_has_leaders:
                            if not is_leader_in_prev_week and not is_leader_in_next_week and is_leader_available:
                                roster.get_datafield("Roster")[date].append(one_time_leaders.pop(0))
                            else:
                                print(one_time_leaders[0].get_datafield("Name"),
                            ...
                            if has_more_than_one_slot_left:
                                if still_has_members:
                                    if not is_member_in_prev_week and not is_member_in_next_week and is_member_available:
                                        roster.get_datafield("Roster")[date].append(one_time_members.pop(0))
                                    else:
                                        print(one_time_members[0].get_datafield("Name"),
                                              'is in prev or next serving week. Date:', date)
                                else:
    ...
    while not is_roster_full(roster, min_serve_members):
        for person in serving_members:
            for d in range(len(serving_dates)):
                ...
                is_last_date = d == len(serving_dates) - 1
                has_more_than_one_slot_left = min_serve_members - len(roster.get_datafield("Roster")[date]) > 1
                is_member = person.get_datafield("Role") == "Member"
                is_leader = not is_member
                is_person_available = person.get_datafield("Name") not in \
                                      block_out_dates.get_datafield("Block Out Dates")[date] and person not in \
                                      roster.get_datafield("Roster")[date]
                is_member_in_prev_week = is_member and (
                        (is_first_date and (person.get_datafield("Name") in last_month_serving_week)) or (
                        (not is_first_date) and (
                        person in roster.get_datafield("Roster")[serving_dates[d - 1]])))
                is_member_in_next_week = is_member and ((is_last_date and (person.get_datafield("Name") in [])) or (
                        (not is_last_date) and (person in roster.get_datafield("Roster")[serving_dates[d + 1]])))
                is_leader_in_prev_week = is_leader and (
                        (is_first_date and (person.get_datafield("Name") in last_month_serving_week)) or (
                        (not is_first_date) and (
                        person in roster.get_datafield("Roster")[serving_dates[d - 1]])))
                is_leader_in_next_week = is_leader and ((is_last_date and (person.get_datafield("Name") in [])) or (
                        (not is_last_date) and (person in roster.get_datafield("Roster")[serving_dates[d + 1]])))
                is_serve_too_much = get_person_serving_amount(roster, person) > get_avg_serving_amount(roster,
                                                                                                       serving_members)

                if is_person_available and not is_serving_week_full(roster.get_datafield("Roster")[date],
                                                                    min_serve_members):
                    if not has_a_leader(roster.get_datafield("Roster")[date]):
                        if is_leader \
                            and (
                            ...
                            roster.get_datafield("Roster")[date].append(person)
                            break
                    else:
    ...
    print(sorted(
        [(person.get_datafield("Name"), get_person_serving_amount(roster, person)) for person in serving_members],
